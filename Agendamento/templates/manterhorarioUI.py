    ...
    def listar():
        ...
        if len(horarios) == 0: st.write("Nenhum horário cadastrado")
            ...
            for obj in horarios:
                ...
                dic.append({"id" : obj.get_id(), "data" : obj.get_data(), "confirmado" : obj.get_confirmado(), "cliente" : cliente, "serviço" : servico, "pagamento" : estado_pagamento, "profissional": profissional})
            df = pd.DataFrame(dic)
            st.dataframe(df)
        return horarios
    ...
    def excluir():
        ...
        if len(horarios) == 0: st.write("Nenhum horário cadastrado")
            ...
            if st.button("Excluir"):
                try:
                    View.horario_excluir(op.get_id(), op.get_data())
